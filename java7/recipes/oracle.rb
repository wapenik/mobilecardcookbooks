...
java_home = node['java7']["java_home"]
arch = node['java7']['arch']
jdk_version = node['java7']['jdk_version']
...
case jdk_version
when "6"
  tarball_url = node['java7']['jdk']['6'][arch]['url']
  tarball_checksum = node['java7']['jdk']['6'][arch]['checksum']
  bin_cmds = node['java7']['jdk']['6']['bin_cmds']
when "7"
  tarball_url = node['java7']['jdk']['7'][arch]['url']
  tarball_checksum = node['java7']['jdk']['7'][arch]['checksum']
  bin_cmds = node['java7']['jdk']['7']['bin_cmds']
end
...
file "/etc/profile.d/jdk.sh" do
  content <<-EOS
    export JAVA_HOME=#{node['java7']['java_home']}
  EOS
  mode 0755
end
