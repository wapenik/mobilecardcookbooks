...
the java6::oracle recipe forces you to set either override
the `node['java6']['oracle']['accept_oracle_download_terms']` to true or set up a
...
* `node["java6"]["install_flavor"]` - Flavor of JVM you would like installed (`oracle` or
`openjdk`), default `openjdk`.
* `node['java6']['java_home']` - Default location of the "`$JAVA_HOME`".
* `node['java6']['tarball']` - Name of the tarball to retrieve from your corporate
repository default `jdk1.6.0_29_i386.tar.gz`
* `node['java6']['tarball_checksum']` - Checksum for the tarball, if you use a different
tarball, you also need to create a new sha256 checksum
* `node['java6']['jdk']` - Version and architecture specific attributes for setting the
URL on Oracle's site for the JDK, and the checksum of the .tar.gz.
* `node['java6']['remove_deprecated_packages']` - Removes the now deprecated Ubuntu JDK
packages from the system, default `false`
* `node['java6']['oracle']['accept_oracle_download_terms']` - Indicates that you accept
  Oracle's EULA
* `node['java6']['windows']['url']` - The internal location of your java install for windows
* `node['java6']['windows']['package_name']` - The package name used by windows_package to
...
Simply include the `java6` recipe where ever you would like Java installed.

To install Oracle flavored Java on Debian or Ubuntu override the `node['java6']['install_flavor']` attribute with in role:
