...
arch = node['java6']['arch']
jdk_version = node['java6']['jdk_version']
...
case jdk_version
when "6"
  tarball_url = node['java6']['jdk']['6'][arch]['url']
  tarball_checksum = node['java6']['jdk']['6'][arch]['checksum']
  bin_cmds = node['java6']['jdk']['6']['bin_cmds']
when "7"
  tarball_url = node['java6']['jdk']['7'][arch]['url']
  tarball_checksum = node['java6']['jdk']['7'][arch]['checksum']
  bin_cmds = node['java6']['jdk']['7']['bin_cmds']
end
...
file "/etc/profile.d/jdk.sh" do
  content <<-EOS
    export JAVA_HOME=#{node['java6']['java_home']}
  EOS
  mode 0755
end
